...
def main():
  ...
  for repo in (g.get_repo("{}/{}".format(os.getenv("GITHUB_ORG"), r)) for r in repo_names):
    ...
    for pr in repo.get_pulls(state="all")[:NUM_PRS]:
      ...
      if DEBUG: print("#{} | merged: {} | state: {} | author: {} | title: {}".format(pr.number, pr.is_merged(), pr.state, pr.user.login, pr.title))
      ...
      for comment in pr.get_comments():
        ...
        if DEBUG: print("    * pr-comment: {}: {}".format(comment.user.login, comment.body.splitlines()[0]))
      ...
      for issue_comment in pr.get_issue_comments():
        ...
        if DEBUG: print("    * issue-comment: {}: {}".format(comment.user.login, comment.body.splitlines()[0]))
      ...
      for review in [review for review in pr.get_reviews() if review.state != "COMMENTED"]:
        ...
        if DEBUG: print("    * pr-review: {}: {}".format(review.user.login, review.state, review.body))
